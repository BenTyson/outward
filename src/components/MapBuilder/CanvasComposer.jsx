...
const CanvasComposer = () => {
  ...
  const generateHighResExport = useCallback(async () => {
    ...
    try {
      ...
      // This creates files optimal for professional laser engraving
      ...
      // Option 2: Also create a compressed JPEG version for smaller file size
      // (useful for email sharing while PNG is for laser engraving)
      setTimeout(() => {
        const jpegDataUrl = canvas.toDataURL('image/jpeg', 0.95); // 95% quality
        const jpegLink = document.createElement('a');
        jpegLink.download = `${baseFilename}-compressed.jpg`;
        jpegLink.href = jpegDataUrl;
        
        // Optional: Auto-download JPEG too (commented out to avoid dual download)
        // jpegLink.click();
        
        console.log('Generated exports:', {
          png: `${Math.round(pngDataUrl.length / 1024)}KB`,
          jpeg: `${Math.round(jpegDataUrl.length / 1024)}KB`,
  ...
  return (
    <div className="canvas-composer">
      ...
      <div className="export-controls">
        ...
        >
          {isExporting ? 'Exporting...' : 'Export Ultra High Resolution (1200 DPI)'}
        </button>
      </div>
      
      <div className="export-info">
        <p>Preview shows your design at screen resolution (800px wide).</p>
        <p>Ultra high-resolution export creates a 1200 DPI image (4800px wide) optimized for professional laser engraving.</p>
        <p><strong>File sizes:</strong> PNG format for laser engraving (~2-8MB), JPEG available for sharing (~200-800KB).</p>
      </div>
    </div>
  );
};
